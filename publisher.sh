...
if [[ $USER_BRANCH == *"group"* ]]; then
  string=$(git branch -a | sed -r "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2};?)?)?[mGK]//g" | grep "$USER_BRANCH$") # acha as branches com o nome definido nos parâmetros caso haja "group" no nome
else
  string=$(git branch -a | sed -r "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2};?)?)?[mGK]//g" | grep "$USER_BRANCH") # acha as branches com o nome definido nos parâmetros  
fi
